defmodule Cldr.Calendar.PropertyTest do
  ...
  property "next and previous weeks" do
    check all date <- Calendar.Date.generate_date(), max_runs: @max_runs do
      ...
      assert Cldr.Calendar.date_to_iso_days(this.last) + 1 == Cldr.Calendar.date_to_iso_days(next.first)
      assert Cldr.Calendar.date_to_iso_days(previous.last) + 1 == Cldr.Calendar.date_to_iso_days(this.first)
    end
  end
