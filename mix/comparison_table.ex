if Code.ensure_loaded?(Cldr.Print) do
  defmodule Compare do
    def print do
      ...
      for year <- 2019..2019, month <- 1..12, day <- 1..Calendar.ISO.days_in_month(year, month) do
        ...
        iso = Cldr.Calendar.iso_week_of_year(date)
        en = Cldr.Calendar.week_of_year(date, locale: "en")
        en_001 = Cldr.Calendar.week_of_year(date, locale: "en-001")

        Cldr.Print.printf(format, [
          to_string(date),
          inspect(erl),
          inspect(iso),
          inspect(en),
